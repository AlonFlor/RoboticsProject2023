...
#create objects
num_masses_to_sample = 4
startPos = (0.0, 0.0, 0.05)
startOrientation = p.getQuaternionFromEuler([0, -np.pi / 2, 0])

model_masses = []
for i in np.arange(num_masses_to_sample):
    model_masses.append(1.+0.5*i)

for i in np.arange(num_masses_to_sample):
    # load object
    objectID, object_shapes_entry = p_utils.load_object("cracker_box", test_dir, (0., 0., 0.06))
    p.changeDynamics(objectID,-1,model_masses[i]) #change object mass
    ...
    p.resetBasePositionAndOrientation(objectID, (startPos[0]-0.06, startPos[1], startPos[2]+0.2*i), startOrientation)

#disable collision between the objects
for i in np.arange(1,num_masses_to_sample+1):
    for j in np.arange(1,i):
        p.setCollisionFilterPair(i,j,-1,-1,0)

#set the objects to be on top of each other
for i in np.arange(num_masses_to_sample):
    p.resetBasePositionAndOrientation(i+1, (startPos[0]+0.08, startPos[1], startPos[2]), startOrientation)

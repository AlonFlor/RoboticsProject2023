...
class node:
    ...
    def generate_action_list(self):
        ...
        for id in mobile_object_IDs:
            basic_id_ply_str = str(self.nodeNum) + "_objID_"+str(id)
            ...
            pcl_get_normals_command = "pclnormal.exe " + id_ply_str + " " + id_ply_with_normals_str
            os.popen(pcl_get_normals_command)
            time.sleep(0.001)
            pcl_get_normals_command = "pclnormal.exe " + id_ply_obstacles_str + " " + id_ply_obstacles_with_normals_str
            os.popen(pcl_get_normals_command)
            time.sleep(0.001)

            id_ply_pushing_points_str = os.path.join(self.node_dir, basic_id_ply_str+"pushing_points.ply")
            pcl_pushing_points_command = "pclpush.exe " + id_ply_with_normals_str + " " + id_ply_obstacles_with_normals_str + " " + id_ply_pushing_points_str
            os.popen(pcl_pushing_points_command)
            time.sleep(0.001)

            id_ply_downsampled_pushing_points_str = os.path.join(self.node_dir, basic_id_ply_str+"downsampled_pushing_points.ply")
            pcl_down_sample_command = "pclsample.exe " + id_ply_pushing_points_str + " " + id_ply_downsampled_pushing_points_str
            os.popen(pcl_down_sample_command)
            time.sleep(0.001)

            candidate_list = p_utils.get_points_from_ply_files(id_ply_downsampled_pushing_points_str)
